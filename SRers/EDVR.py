...
class SRer:
    models = {  # 'model_name': [model, num_of_frame, enlarge_factor:ef, multiple, max_res]
        'ld':   [EDVR(num_feat=128, num_reconstruct_block=40, hr_in=True, with_predeblur=True).cuda(), 5, 1, 16, 994],
        'ldc':  [EDVR(num_feat=128, num_reconstruct_block=40, hr_in=True, with_predeblur=True).cuda(), 5, 1, 16, 994],
        'l4r':  [EDVR(num_feat=128, num_reconstruct_block=40).cuda(), 5, 4, 4, 288],
        'l4v':  [EDVR(num_feat=128, num_reconstruct_block=40, num_frame=7, center_frame_idx=3).cuda(), 7, 4, 4, 256],
        'l4br': [EDVR(num_feat=128, num_reconstruct_block=40, with_predeblur=True).cuda(), 5, 4, 4, 256],
        'm4r':  [EDVR(with_tsa=False).cuda(), 5, 4, 4, 416],
        'mt4r': [EDVR().cuda(), 5, 4, 4, 416]
    }
